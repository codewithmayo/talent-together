      ...
      try {
        ...
        if (profileData) {
          ...
          const typedProfileData: UserProfile = {
            ...
            platforms: Array.isArray(rawData.platforms) ? rawData.platforms : [],
            ...
            email: rawData.email || '',
            phone: rawData.phone || '',
            preferred_contact: (rawData.preferred_contact || 'email') as 'email' | 'phone',
            created_at: rawData.created_at || new Date().toISOString(),
            updated_at: rawData.updated_at || new Date().toISOString(),
            engagement_stats: rawData.engagement_stats 
              ? {
                  likes: Array.isArray(rawData.engagement_stats.likes) ? rawData.engagement_stats.likes as [number, number, number] : [0, 0, 0],
                  comments: Array.isArray(rawData.engagement_stats.comments) ? rawData.engagement_stats.comments as [number, number, number] : [0, 0, 0],
                  analyticsImage: String(rawData.engagement_stats.analyticsImage || ''),
                  hideAnalytics: Boolean(rawData.engagement_stats.hideAnalytics)
                }
              : undefined,
            budget_range: rawData.budget_range || '',
            min_budget: Number(rawData.min_budget) || 0,
            max_budget: Number(rawData.max_budget) || 0,
            collaboration_types: Array.isArray(rawData.collaboration_types) ? rawData.collaboration_types : [],
            preferred_creator_niches: Array.isArray(rawData.preferred_creator_niches) ? rawData.preferred_creator_niches : [],
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 py-8">
        <div className="container px-4">
          ...
          {isCurrentUser && profile.type === 'creator' && <ProfileVisibilityBanner isPublic={profile.is_public || false} />}            <ProfileHeader 
              profile={profile} 
